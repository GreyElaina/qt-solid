...
impl NodeTree {
    ...
    pub fn register(&mut self, handle: u32, kind: NodeKind) -> Result<(), String> {
    ...
    pub fn kind(&self, handle: u32) -> Option<NodeKind> {
        self.nodes.get(&handle).map(|node| node.kind)
    }
...
mod tests {
    ...
    fn insert_before_anchor_updates_sibling_chain() {
        ...
        tree.register(2, NodeKind::Window).unwrap();
        tree.register(3, NodeKind::Window).unwrap();
        tree.register(4, NodeKind::Window).unwrap();
