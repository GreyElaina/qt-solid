...
use objc2_metal::{
    ...
    MTLRenderPipelineState, MTLSamplerAddressMode, MTLSamplerDescriptor,
    MTLSamplerMinMagFilter, MTLSamplerMipFilter, MTLSamplerState, MTLSize,
...
use qt_compositor_gpu::record_compositor_present_decision;
use qt_compositor_types::{QtCompositorError, QtCompositorTarget, Result};
...
use crate::{
    ...
    trace::trace, with_window_compositor_layer_texture_handle,
};
...
fragment float4 quad_fragment(VertexOut in [[stage_in]],
    ...
    return color_texture.sample(color_sampler, in.uv);
}
...
impl Presenter for MetalPresenter {
    fn configure_for_target(
        ...
        configure_metal_layer(target, &self.layer, self.raw_device.as_ref(), present_format)
    }
...
pub(crate) fn retain_protocol_object<P: ?Sized>(
    ...
    unsafe { Retained::retain(ptr) }.ok_or_else(|| QtCompositorError::new(format!("{label} is null")))
}
...
fn quad_vertices_for_rect(
    ...
    let left = (x as f32 / target.width_px.max(1) as f32) * 2.0 - 1.0;
    let right = ((x + width) as f32 / target.width_px.max(1) as f32) * 2.0 - 1.0;
    let top = 1.0 - (y as f32 / target.height_px.max(1) as f32) * 2.0;
    let bottom = 1.0 - ((y + height) as f32 / target.height_px.max(1) as f32) * 2.0;
    [
        QuadVertex {
            position: [left, top],
            uv: [u0, v0],
...
fn render_snapshot_to_drawable(
    ...
    let base_texture = {
        ...
        retain_protocol_object(
            texture as *const ProtocolObject<dyn MTLTexture>
    ...
    record_compositor_present_decision(snapshot.target, true);
    Ok(())
}
...
fn borrowed_protocol_object<P: ?Sized>(ptr: *mut ProtocolObject<P>) -> Option<&'static ProtocolObject<P>> {
