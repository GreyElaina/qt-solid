...
* 简单的在做某件或某系列事之前，描述当前*情景*：
  ...
  * `joke` 情景 (tail-recursion)：这不是一个正式的情景。你正在应付用户的俏皮话，同样以简短的一句俏皮话回应并立刻报告真实的对接下来将做之事的情景。
...
* **必须不能/MUST NOT**想着「*先跑起来*」之类的鬼话，我们要跑起来的不是代码 —— 如果代码以错误的结构组织，就是跑起来了也是纯粹的浪费资源，而这毫无意义。按照规划组织代码，并根据实际情况修正实现细节，仅在必要情况下返回修正规划。
...
* 很好，首先我们不是蛆虫，我们不能到像蛆一样在屎山上到处钻洞来达成目的，这只会把屎山弄塌。你不能到处都采用打洞之类的办法来允许特例。
  * 也即「站得高看得远」，需要更多的从体系架构、组件接线这方面，而不仅仅是从局部逻辑来考虑如何解决问题。
...
* 选用更高效的工具（如 `vitest`、`cargo nextest` 等）以提升测试速度。
...
* 开始工作前检查项目下的 `package.json` 的 scripts、`cargo xtask`、`pyproject.toml`、`zig build --help` 等以优先检查项目工作流规范。
...
6. (By Akashina) 我不知道 codex-cli 给你注入了什么奇怪的 system prompt，而你**不需要**为我提供精确的行号信息，粗略的指出在哪个文件就行。你寻找并推理这点信息的时间和这最终能呈现给我的信息量相比起来实在是不值得。
...
* 对明显具有破坏性的操作（删除/覆盖文件、清理工作区、历史重写、不可逆恢复等）：
  ...
  * 在真正执行这类操作前，**必须**获得 Akashina 明确确认。

* 对“清理无关改动/收敛 diff/回滚看起来不相关的文件”这种语义不清的指令：
  * 在执行任何 `restore/clean/rm` 之前，**必须**先输出并与 Akashina 对齐：
...
* 只使用 fd (fd-find) 与 rg (ripgrep); 使用 rg 时不要使用 `-r` = `--replace` 参数，尤其是 `-rn` = `--replace="n"`。用 rg 时 pattern 的转义要搞对。
...
* 不要盲目的尝试通过 `git stash` 等方法去判定问题是否 pre-existing ，直接处理你更改区域之内的问题，同时遇到这种情况应向我发起确认 —— 因为你可能正在处理一项正进行工作的环节，而不是一项原子任务。
