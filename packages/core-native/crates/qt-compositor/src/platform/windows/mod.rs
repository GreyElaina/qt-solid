...
use crate::types::{
    QT_COMPOSITOR_SURFACE_WIN32_HWND, QtCompositorBaseUpload, QtCompositorLayerUpload,
    QtCompositorTarget, Result,
};

pub fn load_or_create_compositor(target: QtCompositorTarget) -> Result<Arc<dyn Compositor>> {
    compositor_actor::load_or_create_wgpu_compositor("windows", QT_COMPOSITOR_SURFACE_WIN32_HWND, target)
}
