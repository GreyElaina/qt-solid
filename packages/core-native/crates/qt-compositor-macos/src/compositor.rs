...
use qt_compositor_core::{Compositor, CompositorOwner, FrameReason};
...
use crate::{
    ...
    state::{
        ...
        PendingMetalDisplayLinkDrawable, PresentEpochDisposition, compare_present_state,
    },
    trace::trace, with_window_compositor_device_queue,
};
...
impl MacosCompositorActor {
    fn handle_message(&mut self, message: ActorMessage) -> Result<ActorReply> {
        match message {
            ...
            ActorMessage::IngestFrame {
                window_id,
                target,
                base,
                layers,
            } => self
                .ingest_frame(window_id, target, base, layers)
                .map(ActorReply::Bool),
            ...
            ActorMessage::TryPresentIngestedSnapshot => {
                self.try_present_ingested_snapshot().map(ActorReply::Bool)
            }
            ActorMessage::TryPresentDeferred => Ok(ActorReply::Bool(false)),
    ...
    fn ingest_frame(
        ...
        trace(format_args!(
            "ingest window={} target={}x{} epoch={} base_bytes={} layers={}",
            window_id,
            target.width_px,
            target.height_px,
            epoch,
        ...
        self.state.store_snapshot(OwnedCompositorSnapshot {
            window_id,
            target,
            base,
            layers,
    ...
    fn try_present_ingested_snapshot(&mut self) -> Result<bool> {
        let Some(pending_drawable) = self.state.pending_drawable.take() else {
            trace(format_args!("snapshot-present no-drawable"));
            return Ok(false);
        };
        let Some(mut snapshot) = self.state.latest_snapshot.take() else {
            trace(format_args!(
                "snapshot-present no-snapshot keep-drawable {}x{} epoch={}",
                pending_drawable.width_px,
                pending_drawable.height_px,
                pending_drawable.epoch
            ));
            self.state.store_pending_drawable(pending_drawable);
            return Ok(false);
        };
        snapshot.target = self
            .state
            .normalized_target_for_pending_drawable(snapshot.target, Some(pending_drawable));

        match compare_present_state(
            true,
            pending_drawable.epoch,
            pending_drawable.width_px,
            pending_drawable.height_px,
            snapshot.epoch,
            snapshot.target.width_px,
            snapshot.target.height_px,
        ) {
            PresentEpochDisposition::KeepDeferredFrame => {
                trace(format_args!(
                    "snapshot-present mismatch keep-snapshot drawable={}x{}@{} snapshot={}x{}@{}",
                    pending_drawable.width_px,
                    pending_drawable.height_px,
                    pending_drawable.epoch,
                    snapshot.target.width_px,
                ...
                drop_retained_metal_drawable(pending_drawable.drawable_handle);
                self.state.store_snapshot(snapshot);
                return Ok(false);
            }
            PresentEpochDisposition::KeepPendingDrawable => {
                trace(format_args!(
                    "snapshot-present mismatch keep-drawable drawable={}x{}@{} snapshot={}x{}@{}",
                    pending_drawable.width_px,
                    pending_drawable.height_px,
                    pending_drawable.epoch,
                    snapshot.target.width_px,
        ...
        let frame_epoch =
            self.ensure_configured(snapshot.target, wgpu::TextureFormat::Bgra8UnormSrgb)?;
        trace(format_args!(
            "snapshot-present submit window={} target={}x{} epoch={}",
            snapshot.window_id,
            snapshot.target.width_px,
            snapshot.target.height_px,
            frame_epoch
        ...
        self.owner.present_complete(snapshot.window_id);
        Ok(true)
    }
}
...
impl Compositor for MacosCompositorHandle {
    fn present_frame(
        ...
        self.dispatch(ActorMessage::PresentFrame {
            target,
    ...
    fn ingest_frame(
        ...
        self.dispatch_bool(ActorMessage::IngestFrame {
            window_id,
            target,
            base: OwnedQtCompositorBaseUpload::from_borrowed(base),
            layers: layers
    ...
    fn try_present_ingested_snapshot(&self, _target: QtCompositorTarget) -> Result<bool> {
        self.dispatch_bool(ActorMessage::TryPresentIngestedSnapshot)
    }

    fn try_present_deferred(&self, _target: QtCompositorTarget) -> Result<bool> {
        self.dispatch_bool(ActorMessage::TryPresentDeferred)
    }
