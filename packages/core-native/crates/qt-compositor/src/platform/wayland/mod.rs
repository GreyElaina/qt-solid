...
use crate::types::{
    QT_COMPOSITOR_SURFACE_WAYLAND_SURFACE, QtCompositorBaseUpload, QtCompositorLayerUpload,
    QtCompositorTarget, Result,
};

pub fn load_or_create_compositor(target: QtCompositorTarget) -> Result<Arc<dyn Compositor>> {
    compositor_actor::load_or_create_wgpu_compositor("wayland", QT_COMPOSITOR_SURFACE_WAYLAND_SURFACE, target)
}
