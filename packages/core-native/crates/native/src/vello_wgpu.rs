use qt_wgpu_renderer::{
    load_or_create_context, QtNativeTextureLease, QtRhiInteropInfo, TextureCacheKey,
};
use vello::{peniko::Color, AaConfig, RenderParams, Renderer, RendererOptions, Scene};
...
pub(crate) fn render_vello_scene_to_native_texture(
    ...
    let (target_texture, texture_view, texture_lease) = match rhi_interop {
        QtRhiInteropInfo::Metal(_) => {
            ...
            (
                entry.texture_view().texture().clone(),
                entry.texture_view().clone(),
    ...
    match rhi_interop {
        QtRhiInteropInfo::D3d12(_) => {
            qt_wgpu_renderer::d3d::finish_texture_render_d3d12(&context, &target_texture)
                .map_err(|error| qt_error(error.to_string()))?
        }
